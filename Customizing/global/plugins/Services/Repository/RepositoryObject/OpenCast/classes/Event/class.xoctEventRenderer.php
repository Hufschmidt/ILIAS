...
class xoctEventRenderer
{
    ...
    public function insertUnprotectedLink(
        ...
        // Get Links and Resolutions
        list($links, $heights) = $this->event->publications()->getUnprotectedLinks() ?: ['', ''];
        // Build html and sign Links
        ...
        $link_tpl->setVariable('UNPROTECTEDLINK_HTML', $unprotectedlink_html);
        $link_tpl->setVariable('UNPROTECTEDLINK_HTML_INFO', $this->plugin->txt('unprotected_link_info'));
        $this->insert($tpl, $variable, $unprotectedlink_html ? $link_tpl->get() : '', $block_title);
    }
